...
with open('input.txt', 'r') as f:
    ...
    for idx, i in enumerate(data):
        ...
        for jdx, j in enumerate(i):
            if (idx, jdx) in main_loop:
                pipe = data[idx][jdx]
                # Can go with Regex/Replace but To find `S` it was converted to a list.
                if pipe == '|' or (wall_start == 'F' and pipe == 'J') or (wall_start == 'L' and pipe == '7'):
                    inside = not inside
                elif pipe  in 'FL':
                    wall_start = pipe
            elif inside:
